	...
	public void robBoat(int pirate_id,Boat boat,int game_round,boolean let_boat_to_port) {
		...
		if(game_round==2) {
			if(boat.getAvailPosIndex()!=-1) {
				boat.playerPirateAboard(game.getPlayers()[positions[pirate_id].getPid()],2);
				positions[pirate_id].setPid(-1);
			}
		}
		if(game_round==3) {
			boat.setIs_kidnap(true);
			boat.playerPirateAboard(game.getPlayerByID(positions[pirate_id].getPid()),3);
			positions[pirate_id].setPid(-1);
			if(pirate_id==0&&!game.getPirateBoat().ifPosAvail()[1]) {
				boat.playerPirateAboard(game.getPlayerByID(positions[1].getPid()),2);
