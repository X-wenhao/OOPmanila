...
public class Player {
	...
	/**可否上船*/
	private boolean canAboard = true;
	 ...
	 * @return
	 */
	public boolean hasNoMoney(int payMoney) {
		System.out.println(findShareCanPledge().size());
		if(findShareCanPledge().size()<=0) {
			if(account_balance<payMoney) {
				return true;
			}
		}
		return false;
	...
	public boolean payMoney(int money) {
		System.out.println(canAboard);
		if(!canAboard)return false;
		if(hasNoMoney(money)) {
			account_balance = 0;
			canAboard = false;
	...
	//getter setter
	public boolean isCanAboard() {
		return canAboard;
	}

	public void setCanAboard(boolean canAboard) {
		this.canAboard = canAboard;
	}
