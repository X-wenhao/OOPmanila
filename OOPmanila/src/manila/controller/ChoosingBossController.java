...
public class ChoosingBossController implements ActionListener {
	...
	public void confirm(){
		...
		this.cbv.setAlwaysOnTop(false);
	}
	
	/**
	 * 选择完船老大的操作
	 */
	public void hasChosenBoss() {
		cbv.getGameView().updatePlayersView(0, false);
		cbv.getGameView().updatePlayersView(cbv.getGame().getCurrent_pid(), true);
		//cbv.getGameView().getGame().setIs_choosing(true);
		cbv.getGameView().getBossInteractor().setVisible(true);

		cbv.getGameView().addGameInfo(cbv.getGameView().getGame().getBoss().getPlayer().getName()+"成为船长，为所欲为...\n");
	}
